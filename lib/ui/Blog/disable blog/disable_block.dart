...
class DisableBlog extends StatefulWidget {
  int userId;
  bool is_block;
  DisableBlog({
    Key? key,
  ...
  @override
  State<DisableBlog> createState() => _DisableBlogState();
}

class _DisableBlogState extends State<DisableBlog> {
  @override
  void initState() {
    super.initState();
  }
                      ...
                      child: SmallText(
                        text: widget.is_block == true
                            ? AppLocalizations.of(context)!.translate(
                                "Are you sure form Activation this blog")
                            : AppLocalizations.of(context)!.translate(
                                "Are you sure form blocking block",
                              ),
  ...
  disableServicesMethed() async {
    ...
    final response = await put(
      Uri.parse(widget.is_block == true
          ? "${Urls.DISABLE_BLOG_URL}${widget.userId}/active"
          : "${Urls.DISABLE_BLOG_URL}${widget.userId}/block"),
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (_) => BlogScreen()));
